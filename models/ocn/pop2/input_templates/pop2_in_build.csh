...
#jj note: ew_boundary_type should be closed or open for the Baltic Sea
#jj it should be checked later
#jj in the cesm version ew_boundary_type was hardwired to 'cyclic'
#jj maybe there is a bug
...
&domain_nml
  ...
  ew_boundary_type         = '$ew_boundary_type'
...
&diagnostics_nml
   ...
   diag_cfl_freq          = 1
   diag_transp_freq_opt   = '\$tdiag_freq_opt'
   diag_transp_freq       = 1
...
if (${OCN_GRID} =~ tx0.1*  || ${OCN_GRID} =~ tx1* || ${OCN_GRID} =~ bs*) then
...
if ( ${OCN_GRID} =~ gx* ) then
  ...
  set n_tavg_streams               = 3
  set ltavg_streams_index_present  = .true.
  set tavg_freq_opt_values         = ("'nhour'" "'never'"   "'never'"                       )
  set tavg_freq_values             = (    1         1           1                           )
  set tavg_stream_filestrings      = ("'hydro.stream1'" "'hydro.stream2'" "'hydro.stream3'" )
  set tavg_file_freq_opt           = ("'nhour'" "'never'" "'never'"                         )
  set tavg_file_freq_values        = (    1         1           1                           )
  set tavg_start_opt_values        = ("'nstep'"   "'nstep'" "'nstep'"                       )
  set tavg_start_values            = (    0         0           0                           )
  set tavg_fmt_in_values           = ("'nc'"      "'nc'"      "'nc'"                        )
...
set vmix_choice = kpp
set implicit_vertical_mix = .true.
set convection_type = diffusion
set convect_diff = 1000.0
set convect_visc = 1000.0
#jj note: currnetly it is Richardson vertical mixing parametrization
#jj but finnaly it should be kpp
...
cat >> $POP2BLDSCRIPT << EOF2
&vertical_mix_nml
   vmix_choice           = '$vmix_choice'
   aidif                 = 1.0
   implicit_vertical_mix = $implicit_vertical_mix
   convection_type       = '$convection_type'
   nconvad               = 2
   convect_diff          = $convect_diff
   convect_visc          = $convect_visc
   bottom_drag           = 1.0e-3
   bottom_heat_flx       = 0.0
...
if (${OCN_GRID} =~ bs* ) then
  set bckgrnd_vvc = 0.2
  set bckgrnd_vdc = 0.05
endif

cat >> $POP2BLDSCRIPT << EOF2
&vmix_rich_nml
   bckgrnd_vvc = $bckgrnd_vvc
   bckgrnd_vdc = $bckgrnd_vdc
   rich_mix    = 50.0
/
...
#jj note: tides should be added in 2 km resolution model in the straits area
...
set lrich                  = .true.
set ldbl_diff              = .true.
set lshort_wave            = .true.
set lcheckekmo             = .false.
...
if ( ${OCN_GRID} == gx3v5 || ${OCN_GRID} == gx3v6 ) then
 ...
 set llangmuir = .false.
 set linertial = .false.
 set bckgrnd_vdc1 = 0.02
 set bckgrnd_vdc2 = 0.0
 set bckgrnd_vdc_dpth = 250000.0e02
 set bckgrnd_vdc_eq     = 0.0
 set bckgrnd_vdc_psim   = 0.0
 set bckgrnd_vdc_ban    = 0.0
 set ldbl_diff = .false.
 set lshort_wave = .false.
 set lcheckekmo  = .true.
...
&vmix_kpp_nml
   ...
   lrich                  = .true.
   ldbl_diff              = $ldbl_diff
   lshort_wave            = $lshort_wave
   lcheckekmo             = $lcheckekmo
   num_v_smooth_Ri        = 1
...
&forcing_pt_interior_nml
   ...
   pt_interior_filename          = '/users/kdm/anowicki/CESM/inputdata/ocn/pop/bs2v1/forcing/interior/temp_3d_bs2v1_monthly_20120217.ieeer8'
...
&assim_pt_interior_nml
   ...
   assim_pt_interior_restore_max_level = 21 
   assim_pt_interior_formulation       = 'restoring'
   assim_pt_interior_data_renorm(1)    = 1.
...
#jj  set coupled_freq_opt = nhour
#jj  @ coupled_freq = 24 / $OCN_NCPL
if (${OCN_NCPL} > 24) then
   set coupled_freq_opt = nsecond
   @ coupled_freq = 86400 / $OCN_NCPL
else
...
set sw_absorption_type = chlorophyll
if ( ${OCN_GRID} =~ bs9* ) then
...
cat >> $POP2BLDSCRIPT << EOF2
&sw_absorption_nml
   sw_absorption_type = '$sw_absorption_type'
